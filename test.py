...
def main(args):
    ...
    checkpoint_callback = ModelCheckpoint(
        #dirpath=save_path,
        filename= "./checkpoint/" + ckpt_name + " .ckpt",
        save_top_k=1,
        save_last= False,
        monitor="val_loss",
        mode='min',
    ...
    state_dict = torch.load(os.path.join("./checkpoint", ckpt_name + ".ckpt"))
    ...
    with open(os.path.join("./inference/", "evaluate_" + ckpt_name + ".json"), mode="w") as io:
        json.dump(model.test_results, io, indent=4)
